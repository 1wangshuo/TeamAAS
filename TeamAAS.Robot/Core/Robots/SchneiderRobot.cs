...
{
    public class SchneiderRobot : IRobot, INotifyPropertyChanged
    {
        ...
        public SchneiderRobot(RobotInfo robot)
        {
            Id = robot.Id;
        ...
        public RPoint GetRobotPos()
        {
            ...
            {
                CanExecute = false;
                string rev = SendAndReceive($"GetRobotPos");
                if (rev.Contains("GetRobotPos"))
                {
                    ...
                    point.V = Convert.ToSingle(items[5]);
                    point.W = Convert.ToSingle(items[6]);
        ...
        public async Task<RPoint> GetRobotPosAsync()
        {
            ...
            {
                CanExecute = false;
                string rev = await SendAndReceiveAsync($"GetRobotPos");
                if (rev.Contains("GetRobotPos"))
                {
                    RPoint point = new RPoint();
                    var items = rev.Split(',');
        ...
        public async Task<bool> GoAsync(RPoint position)
        {
            ...
            {
                CanExecute = false;
                string rev = await SendAndReceiveAsync($"Go,{Math.Round(position.X, 3)},{Math.Round(position.Y, 3)},{Math.Round(position.Z, 3)},{Math.Round(position.U, 3)},{Math.Round(position.V, 3)},{Math.Round(position.W, 3)},{(int)position.Hand},{position.Local},{position.Tool}");
                if (rev.Contains("Go"))
                    return true;
                else
                    return false;
        ...
        public bool Jump(RPoint position, double? LimZ)
        {
            try
            {
                ...
                string rev = SendAndReceive($"Jump,{Math.Round(position.X, 3)},{Math.Round(position.Y, 3)},{Math.Round(position.Z, 3)},{Math.Round(position.U, 3)},{Math.Round(position.V, 3)},{Math.Round(position.W, 3)},{(int)position.Hand},{position.Local},{position.Tool},{Math.Round(limz, 3)}");
        ...
        public async Task<bool> JumpAsync(RPoint position, double? LimZ)
        {
            try
            {
                CanExecute = false;
                double limz = LimZ == null ? 0 : (double)LimZ;
                string rev = await SendAndReceiveAsync($"Jump,{Math.Round(position.X, 3)},{Math.Round(position.Y, 3)},{Math.Round(position.Z, 3)},{Math.Round(position.U, 3)},{Math.Round(position.V, 3)},{Math.Round(position.W, 3)},{(int)position.Hand},{position.Local},{position.Tool},{Math.Round(limz, 3)}");
                if (rev.Contains("Jump"))
                    return true;
                else
                    return false;
        ...
        public async Task<bool> MoveAsync(RPoint position)
        {
            ...
            {
                CanExecute = false;
                string rev = await SendAndReceiveAsync($"Move,{Math.Round(position.X, 3)},{Math.Round(position.Y, 3)},{Math.Round(position.Z, 3)},{Math.Round(position.U, 3)},{Math.Round(position.V, 3)},{Math.Round(position.W, 3)},{(int)position.Hand},{position.Local},{position.Tool}");
                if (rev.Contains("Move"))
                    return true;
                else
                    return false;
        ...
        public async Task<bool> CalibMotionAsync(RPoint position, double? LimZ)
        {
            try
            {
                CanExecute = false;
                double limz = LimZ == null ? 0 : (double)LimZ;
                string rev = await SendAndReceiveAsync($"CalibMotion,{Math.Round(position.X, 3)},{Math.Round(position.Y, 3)},{Math.Round(position.Z, 3)},{Math.Round(position.U, 3)},{Math.Round(position.V, 3)},{Math.Round(position.W, 3)},{(int)position.Hand},{position.Local},{position.Tool},{Math.Round(limz, 2)}");
                if (rev.Contains("CalibMotion"))
                    return true;
                else
                    return false;
        ...
        public async Task<bool> SetToolAsync(int index, double x, double y)
        {
            ...
            {
                CanExecute = false;
                string rev = await SendAndReceiveAsync($"SetTool,{index},{Math.Round(x, 3)},{Math.Round(y, 3)}");
                if (rev.Contains("SetTool"))
                    return true;
                else
                    return false;
